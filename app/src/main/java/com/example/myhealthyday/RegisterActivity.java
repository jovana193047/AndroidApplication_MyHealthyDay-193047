...
public class RegisterActivity extends AppCompatActivity {
    ...
    EditText edUsername, edEmailAddress, edPassword, edConfirm;
    Button btnRegister;
    TextView textViewExist;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edUsername = findViewById(R.id.editTextPersonNameBook);
        edEmailAddress = findViewById(R.id.addressBooking);
        edPassword = findViewById(R.id.textPinCode);
        btnRegister = findViewById(R.id.buttonBooking);
        textViewExist = findViewById(R.id.existingUser);

        textViewExist.setOnClickListener(new View.OnClickListener() {
            @Override
                ...
                if(username.length()==0 || emailAddress.length()==0 || password.length()==0 || confirmPassword.length()==0){
                    ...
                    if(password.compareTo(confirmPassword)==0) {
                        if(isValidPassword(password)){
                            db.register(username, emailAddress, password);
